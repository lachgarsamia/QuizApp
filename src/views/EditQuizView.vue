    ...
    <div class="quiz-container">
        <form @submit.prevent="submitQuiz" class="form-container">
            <div class="create-quiz">
                ...
                <h2 class="title">Edit your Quiz</h2>
                <input type="submit" value="Edit Quiz" class="submit-button" />
            </div>

            <div class="quiz-info">
                <h2>Quiz Information</h2>
                <input v-model="title" id="title" type="text" placeholder="Enter quiz title" required
                    class="form-control" />
            ...
            <div v-if="selectedQuestionIndex !== null" class="edit-question">
                ...
                <input v-model="selectedQuestion.text" type="text" placeholder="Enter question text" required
                    class="form-control" />
                <ul>
                    <li v-for="(response, responseIndex) in selectedQuestion.options" :key="responseIndex"
                        class="response-item">
                        <font-awesome-icon @click="addResponse(selectedQuestionIndex)" class="add-response-button"
                            :icon="['fas', 'plus']" />
                        <input v-model="response.text" type="text"
                            :placeholder="'Enter response ' + (responseIndex + 1)" required class="form-control" />
                        <input type="checkbox" class="correct" :value="response.text"
                            v-model="selectedQuestion.correct" />
                        <font-awesome-icon class="remove-response-button"
                            @click="removeResponse(responseIndex, selectedQuestionIndex)"
...
export default {
    ...
    async created() {
        ...
        try {
            ...
            this.description = editquiz.description;
            this.questions = editquiz.questions.map((question) => {
                return {
                    text: question.text,
